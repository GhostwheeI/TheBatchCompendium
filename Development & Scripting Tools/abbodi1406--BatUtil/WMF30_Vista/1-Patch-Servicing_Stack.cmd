...
if exist "%_ssu%\CbsCore.old" if exist "%SysPath%\windrust.dll" (
echo %_wrn%
echo Servicing stack is already patched.
goto :TheEnd
)
...
cd %xOS%\
for %%# in (
NSudoLC.exe bbe.exe crypt96.dll windrust.dll
) do if not exist "%%~#" (
echo %_err%
...
@cls
echo.
choice /C YN /N /M "Patch servicing stack? [y/n]: "
...
reg add "%_WUA%" /f /v AUOptions /t REG_DWORD /d 0x1 >nul 2>&1
reg add "%_WUA%" /f /v AUOptions_bak /t REG_DWORD /d %_ebak% >nul 2>&1
call :StopService BITS >nul 2>&1
call :StopService TrustedInstaller >nul 2>&1
call :StopService wuauserv >nul 2>&1

copy /y "%_ssu%\CbsCore.dll" CbsCore.old >nul 2>&1
copy /y "%_ssu%\wcp.dll" wcp.old >nul 2>&1
bbe.exe -e "s/WINTRUST.dll/WINDRUST.dll/" -e "s/wintrust.dll/windrust.dll/" -o CbsCore.dll CbsCore.old >nul 2>&1
bbe.exe -e "s/CRYPT32.dll/CRYPT96.dll/" -e "s/crypt32.dll/crypt96.dll/" -o wcp.dll wcp.old >nul 2>&1
move /y "%_ssu%\CbsCore.dll" "%_ssu%\CbsCore.old" >nul 2>&1
move /y "%_ssu%\wcp.dll" "%_ssu%\wcp.old" >nul 2>&1
move /y CbsCore.dll "%_ssu%\CbsCore.dll" >nul 2>&1
move /y wcp.dll "%_ssu%\wcp.dll" >nul 2>&1
copy /y windrust.dll "%SysPath%\windrust.dll" >nul 2>&1
copy /y crypt96.dll "%SysPath%\crypt96.dll" >nul 2>&1
del /f /q *.old >nul 2>&1
