...
if /i "%~1"=="--autorun-userinit" (
	...
	(tasklist /fi "imagename eq explorer.exe" /v | findstr /c:"%username%" > nul 2>&1) && (
...
:real_time
set counter=0
for /f "tokens=1* delims=:" %%A in ('WaitDirChange /any /s /f !homedrive!\ 2^>nul') do (
	set /a counter+=1
...
set "hash=%~1"
set "hash=!hash:\=!"
for /f "tokens=1* delims=:" %%A in ('findstr /bc:"!hash!" "%~dp0VirusDataBaseHash.bav"') do call :detection "!hash!" "%%~B"
exit /b
...
if "%runningInBackground%"=="1" (
	call :balloon "Threat found: %~2^___^&vbCrLf^&___File: !filescan!" "Batch Antivirus" Warning
) else (
	call :balloon "Threat found: %~2`nFile: !filescan!" "Batch Antivirus" Warning
)
...
certutil -encode -f "%~dp0Data\Quarantine\!hash!\!hash!.tmp" "%~dp0Data\Quarantine\!hash!\!hash!" > nul 2>&1 && del "%~dp0Data\Quarantine\!hash!\!hash!.tmp" /q /f > nul 2>&1
...
if "%is_process%"=="1" taskkill /f /im "!fs_basename!" > nul 2>&1 && (echo Successfully killed the malicious process '!fs_basename!') || (echo Failed to kill the malicious process '!fs_basename!')
...
for /f "tokens=2,3 delims=:" %%A in ('netstat -no ^| findstr /vc:"127.0.0.1"') do (
	...
	for /f "tokens=2* delims=:" %%A in ('findstr /bc:"!ip!" "%~dp0VirusDataBaseIP.bav" "%~dp0Data\BlockedSites.bav" 2^>nul') do (
...
if "!show_balloon!"=="1" if "%runningInBackground%" neq "1" start /b powershell [Reflection.Assembly]::LoadWithPartialName("""System.Windows.Forms""");$obj=New-Object Windows.Forms.NotifyIcon;$obj.Icon = [System.Drawing.SystemIcons]::%~3; $obj.BalloonTipIcon = """%~3""";$obj.Visible = $True;$obj.ShowBalloonTip(%balloon_notification_timeout%, """%~2""","""%~1""",2) > nul
...
for /f %%A in ('wmic /namespace:\\root\cimv2 path Win32_PerfFormattedData_Counters_ThermalZoneInformation get Temperature  2^>nul ^| findstr /rc:"[0-9]"') do (
	...
	if !temperature! geq 90 if !display_overheat! equ 1 (
		echo.WARNING: CPU overheating detected ^(!temperature!C^). It is recommended to shut down the computer.
		call :balloon "WARNING: CPU overheating detected ^(!temperature!C^). It is recommended to shut down the computer." "Batch Antivirus" Warning
...
if "!kill_protection!"=="0" (
	(tasklist /fi "imagename eq wscript.exe" | findstr /c:"=" > nul 2>&1 ) || (
		echo.Kill protection process was killed^^! Restarting...
		set kill_protection=1
		call :killprot
	)
)
if "!kill_protection!"=="1" (
	...
	echo.createObject^(^"WScript.Shell^"^).Run ^"cmd /c ^"^"%~f0^"^"^", vbHide, false >> "!TMP!\!kp_file!"
	echo do >> "!TMP!\!kp_file!"
	...
	echo 		MsgBox "The real-time protection module unexpectedly terminated. The process is now going to be restarted.", 4112, "Batch Antivirus Kill Protection" >> "!TMP!\!kp_file!"
