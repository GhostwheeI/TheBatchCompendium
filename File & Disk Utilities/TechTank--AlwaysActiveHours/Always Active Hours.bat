@echo off & for /f "tokens=2 delims=:" %%a in ('chcp') do set /a "codepage=%%a" & chcp 65001 >nul & set "EOL_REPAIRED=0" & goto :eol_verify
...
(
	...
	echo       ^<Arguments^>/c ""%targetDir%\Always Active Hours.bat" /task"^</Arguments^>
	echo     ^</Exec^>
	echo   ^</Actions^>
	echo ^</Task^>
) > "%xmlPath%"
...
if not exist "%filePath%" (
	if "%silent%"=="false" (
		...
		echo Could not copy Always Active Hours.bat to:
		echo %filePath%
	)
	exit /b 1
)
...
:: Use a multi-byte character to offset the Command Prompt parsing buffer for
:: both single and double byte character sets, causing the subsequent line to
:: either parse as flag or as a failed command used to verify line endings
:: and repair the file if necessary
::
:: A buffer block may be used to clear the Command Prompt's parsing buffer
:: after the multi-byte character
