...
"rstrui.exe"
        ...
        {
            string codeContent = Properties.Resources.Code;
            ...
            Panel searchPanel = new Panel { Dock = DockStyle.Top, Height = 35, BackColor = Color.FromArgb(28, 28, 28) };
            ...
            RichTextBox txtMain = new RichTextBox
            {
                Dock = DockStyle.Fill,
                ReadOnly = true,
                BackColor = Color.FromArgb(30, 30, 30),
            ...
            void SyncScroll()
            {
                int firstChar = txtMain.GetCharIndexFromPosition(new Point(0, 0));
                int firstLine = txtMain.GetLineFromCharIndex(firstChar);

                lineNumbers.Clear();
                StringBuilder sb = new StringBuilder();
                for (int i = 0; i < 70; i++)
                {
                    sb.AppendLine((firstLine + i + 1).ToString());
                }
                lineNumbers.Text = sb.ToString();
            }

            txtMain.VScroll += (s, e) => SyncScroll();

            void ApplyColoring()
            {
                int originalSel = txtMain.SelectionStart;

                Highlight(txtMain, new[] { "using", "namespace", "public", "private", "partial", "class", "void", "static", "new", "if", "return", "true", "false" }, Color.FromArgb(86, 156, 214));

                Highlight(txtMain, new[] { "string", "int", "bool", "var", "Task", "Form", "Button", "TextBox", "IntPtr" }, Color.FromArgb(78, 201, 176));

                HighlightRegex(txtMain, "\".*?\"", Color.FromArgb(214, 157, 133));
                HighlightRegex(txtMain, @"//.*", Color.FromArgb(87, 166, 74));

                txtMain.SelectionStart = originalSel;
            ...
            searchBox.KeyDown += (s, e) => {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    int found = txtMain.Text.IndexOf(term, lastFound, StringComparison.OrdinalIgnoreCase);
                    if (found == -1) found = txtMain.Text.IndexOf(term, 0, StringComparison.OrdinalIgnoreCase);

                    if (found != -1)
                    {
                        txtMain.SelectionStart = found;
                        txtMain.SelectionLength = term.Length;
                        txtMain.ScrollToCaret();
                        txtMain.Focus();
                        lastFound = found + term.Length;
            ...
            sourceForm.Load += (s, e) => { ApplyColoring(); SyncScroll(); };
            sourceForm.Controls.Add(txtMain);
            sourceForm.Controls.Add(lineNumbers);
            sourceForm.Controls.Add(searchPanel);
            sourceForm.Show();
        }

        void Highlight(RichTextBox rtb, string[] words, Color color)
        {
            foreach (var word in words)
            {
                int start = 0;
                while ((start = rtb.Find(word, start, RichTextBoxFinds.WholeWord)) != -1)
                {
        ...
        void HighlightRegex(RichTextBox rtb, string pattern, Color color)
        {
...

            {
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    if (isoupgrade != null)
                    {
                        ...
                        isoupgrade = isodrive.RootDirectory.FullName;
